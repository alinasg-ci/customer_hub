      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50">
            ...
            <th className="px-4 py-2 text-right font-medium text-gray-600">Quoted</th>
            <th className="px-4 py-2 text-right font-medium text-gray-600">Internal</th>
        ...
        <tbody className="divide-y divide-gray-100">
          {rows.map(({ phase, quotedHours, internalHours, delta, overBudget }) => (
            <tr key={phase.id} className={cn(overBudget && 'bg-red-50')}>
              ...
              <td className="px-4 py-2 text-right text-gray-700">{quotedHours}h</td>
              <td className="px-4 py-2 text-right text-gray-700">{internalHours}h</td>
              <td className={cn(
        ...
        <tfoot>
          <tr className="border-t-2 border-gray-200 bg-gray-50 font-semibold">
            ...
            <td className="px-4 py-2 text-right text-gray-900">{totalQuoted}h</td>
            <td className="px-4 py-2 text-right text-gray-900">{totalInternal}h</td>
            <td className={cn(
              'px-4 py-2 text-right',
              totalDelta > 0 ? 'text-red-600' : totalDelta < 0 ? 'text-green-600' : 'text-gray-400'
            )}>
