      ...
      >
        <td className={`py-2 pr-2 ${INDENT[row.level]}`}>
          <div className="flex items-center gap-1">
            ...
            <PlanningRowEditor
              ...
              className={row.level === 1 ? 'font-semibold' : row.level === 3 ? 'text-xs text-gray-600' : ''}
              onSave={(name) => onUpdate(row.id, { name })}
            />

            {row.linked_phase_id && (
              <span className="ml-1 text-blue-500" title="Linked to hours phase">
        ...
        <td className="py-2 px-2">
          <input
            ...
            className="w-28 rounded border border-gray-200 px-1.5 py-1 text-xs text-gray-600"
          />
        </td>
